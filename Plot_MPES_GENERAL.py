...
kx_traces, ky_traces = [0.2, 0.2], [0] # kx, ky for plotting
E_traces = [1.35, 2.1] # Energies for Plotting
kx_int, ky_int, E_int  = 0.4, .5, 0.2 #Integration Ranges

trace_colors = ['black', 'red']
...
for t in np.arange(len(E_traces)):
    ...
    trace = trace - np.mean(trace[6:12])
...
ax[0].set_title('$E$ = ' + str((E_traces[0])) + ' eV', fontsize = 18)
